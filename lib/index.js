...
// src/checkEqualDependencies.ts
function checkEqualDependencies(deps, prevDeps) {
...
// src/getCurrentPackageJson.ts
var import_github2 = __toESM(require_github());

// src/getPackageJson.ts
var core = __toESM(require_core());
var import_github = __toESM(require_github());
async function getPackageJson(ref, token) {
  const octokit = (0, import_github.getOctokit)(token);
  core.debug(`repo: ${JSON.stringify(import_github.context.repo)}`);
  core.debug(`sha: ${ref}`);
  const contents = await octokit.rest.repos.getContent({ ...import_github.context.repo, ref, path: "package.json" });
  const base64Data = contents.data.content;
  if (!base64Data) {
    throw new Error(`Could not find package.json for commit ${ref}`);
  }
  const packageJson = JSON.parse(Buffer.from(base64Data, "base64").toString());
  if (!assertPackageJson(packageJson)) {
    throw new Error(`Did not get valid package json data from sha ${ref}`);
  }
  return packageJson;
}
var assertPackageJson = (data) => {
...
// src/getCurrentPackageJson.ts
function getCurrentPackageJson(token) {
  const commitSha = import_github2.context.sha;
...
var getDependencies = (packageJson) => {
  ...
  return Object.keys(devDependencies).concat(Object.keys(devDependencies)).reduce((acc, name) => {
    acc[name] = dependencies[name] || devDependencies[name];
    return acc;
  }, {});
};

// src/getPreviousPackageJson.ts
var import_github3 = __toESM(require_github());
async function getPreviousPackageJson(token) {
  const octokit = (0, import_github3.getOctokit)(token);
  const commit = await octokit.rest.git.getCommit({ ...import_github3.context.repo, commit_sha: import_github3.context.sha });
...
async function run() {
  try {
    ...
    core2.debug(`Current package.json: ${currentPackageJson.name ?? ""}@${currentPackageJson.version ?? ""}`);
    core2.debug(`Previous package.json: ${previousPackageJson.name ?? ""}@${previousPackageJson.version ?? ""}`);
    ...
    core2.setOutput("has-updated", !hasEqualDependencies);
    core2.setOutput("previous-dependencies", previousDependencies);
    core2.setOutput("dependencies", dependencies);
  } catch (error) {
